...
class Sampler(torch.utils.data.Sampler):
    ...
    def set_epoch(self, epoch):
        self.epoch = epoch
"""
def datafold_read_old(datalist,
                  basedir,
                  fold=0,
                  key='training'):
    ...
    return tr, val
"""
def datafold_read(datalist, basedir, fold=0, key=None):
    with open(datalist) as f:
        json_data = json.load(f)
    ...
    if "training" in json_data and "validation" in json_data:
        ...
        json_data = json_data[key if key else "training"]
        train_data = []
        val_data = []
        for d in json_data:
            if 'fold' in d and d['fold'] == fold:
    ...
    # Convert relative paths to absolute using basedir
...
def get_loader(args):
    ...
    train_transform = transforms.Compose(
        [
            ...
            transforms.CopyItemsd(keys=["image"], times=1, names=["image_path"]),  # ⬅️ Store path
            ...
            transforms.EnsureTyped(keys=["image", "label"]),  # make sure metadata sticks
            transforms.ToTensord(keys=["image", "label"],),
            # transforms.SqueezeDimd(keys=["label"]),
            transforms.Lambdad(keys=["image", "label"], func=identity, overwrite=True),
        ]
    )
