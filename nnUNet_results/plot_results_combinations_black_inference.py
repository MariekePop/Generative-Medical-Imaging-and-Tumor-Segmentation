...
LEGACY = [
    ...
    ("resultsorgpost",        "Original",          "#0343df"),
    ("resultsorgGT2",         "Original 2nd GT (raw)",    "#a2cffe"),
    ("resultsorgpostGT2",     "Original 2nd GT",   "#0165fc"),
    ("resultsextra",          "Extra Data (raw)",              "#fdaa48"),
    ("resultsextrapost",      "Extra Data",             "#ff5b00"),
    ("resultsextraGT2",       "Extra Data 2nd GT (raw)",       "#f9c894"),
    ("resultsextrapostGT2",   "Extra Data 2nd GT",      "#f97306"),
]
...
BLACK_MAP = {
    ...
    647: "ADC black",
    648: "DWI + ADC black",
    651: "30 % T2 + 30 % STIR black",
    652: "T2/STIR + all DWI/ADC black",
    653: "T2/STIR + 60 % DWI/ADC black",
    654: "30 % T1 + 30 % T1C black",
    655: "T1/T1C + 60 % DWI/ADC black",
    656: "T1/T1C + DWI/ADC + T2/STIR black",
}

BLACK_COLOURS = {
    641: "#FF007F",  # pick anything you like
    642: "#FF007F",
    643: "#FF007F",
    644: "#FF007F",
    645: "#FF007F",
    ...
    655: "#FF007F",
    656: "#FF007F",
    # … leave others blank to fall back to PALETTE_RAW/POST
}
...
combo_re = re.compile(r"(\d{3})_(raw|post)(?:_GTreal)?$")
...
for jf in RESULTS_DIR.glob("*.json"):
    ...
    name_dict = COMB_MAP | BLACK_MAP          # merge the two dictionaries
    if ds_id not in name_dict:
        continue

    pretty = f"{name_dict[ds_id]} ({'raw' if stage=='raw' else 'post'})"
    if ds_id in COMB_MAP:                       # generated mixes
        clr = COMB_COLOURS.get(ds_id,
                            PALETTE_RAW if stage == "raw" else PALETTE_POST)
    else:                                       # black-input mixes
        clr = BLACK_COLOURS.get(ds_id,
                                PALETTE_RAW if stage == "raw" else PALETTE_POST)
    colour_map[pretty] = clr
...
for ds in sorted(BLACK_MAP):
    order += [f"{BLACK_MAP[ds]} (raw)", f"{BLACK_MAP[ds]} (post)"]
...
for metric in METRICS:
    ...
    ax.set_title(textwrap.fill(f"{metric} Original vs All-Black vs Single-Sequence-Black", 60),
