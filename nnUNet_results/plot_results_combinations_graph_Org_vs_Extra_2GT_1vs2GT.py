...
LEGACY = [
    ...
    ("metrics",                "1st GT vs 2nd GT",          "#964B00"),
]
...
for metric in METRICS:
    ...
    for x, lbl in enumerate(present):
        ...
        ax.plot([x-BOX_W/2, x+BOX_W/2], [med]*2, color="black", lw=2.2)
        ax.scatter(x, mean, marker="D", s=30,
                   facecolor="white", edgecolor="black", zorder=5)

    ax.set_title(textwrap.fill(f"{metric} Original Dataset vs Extra Data on Both Ground Truths + the two GTs", 60),
    ...
    # handles = [
    #     plt.Line2D([], [], marker='D', linestyle='None', mfc='white', mec='black', label='Median'),
    #     plt.Line2D([], [], color='black', lw=3, label='Mean')
    # ]
