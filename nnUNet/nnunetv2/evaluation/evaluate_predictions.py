...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
	...
	# Convert masks to binary numpy arrays for surface-based metrics
        mask_ref_np = mask_ref.astype(np.bool_)
        mask_pred_np = mask_pred.astype(np.bool_)
	...
	# add HD95 and ASSD
        ...
        if np.any(mask_ref_np) and np.any(mask_pred_np):  # Ensure the masks are not empty
            results['metrics'][r]['HD95'] = hd95(mask_pred_np, mask_ref_np)
            results['metrics'][r]['ASSD'] = assd(mask_pred_np, mask_ref_np)
        else:
